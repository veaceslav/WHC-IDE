...
Highlighter::Highlighter(QString filetype, EditorSettingsVar *es)
{
    ...
    QFile file_f(QString(HIGHLIGHTER_FORMAT_FOLDER) + filetype + QString(".lang"));
    if(!file.exists() || !file_f.exists()){
        ...
        if(!file.open(QFile::ReadOnly) || !file_f.open(QFile::ReadOnly)){
#ifndef DEBUG
            QMessageBox::warning(0,
                                 tr("Warning!"),
            ...
            if(!doc.setContent(&file, &errorMsg, &errorLine, &errorColumn)){
                ...
                QMessageBox::warning(0,
                                     ...
                                     tr("Error parsing the syntax highlighter definitions,\n"
                                        "%1 at line %2, column %3").arg(errorMsg).arg(errorLine).arg(errorColumn));
#endif
...
SyntaxHighlighter *Highlighter::highlight(MdiSubWindow *mdiSubWindow){
    ...
    QDir * dr = new QDir(".");
    Q_UNUSED(dr);
    sh = new SyntaxHighlighter(qobject_cast<MdiTextEditor*>(mdiSubWindow->widget())->document(),
                               container,
                               es);
    return sh;
}
...
void Highlighter::readFormat(const QDomDocument &doc)
{
    ...
    while( !pItemRoot.isNull() ){
        if(pItemRoot.tagName() == "color" && pItemRoot.attribute("format") == "#"){
            QTextCharFormat text;
            text.setForeground(QBrush(QColor(pItemRoot.text())));
            styles.append(new QPair<QString, QTextCharFormat>(pItemRoot.attribute("name"), text));
        }
...
void Highlighter::readLanguages(const QDomDocument &doc)
{
    ...
    while( !pItemRoot.isNull() ){
        ...
        if(pItemRoot.nodeName() == "context"){
            ...
            types.append(new QPair<QString, QVector<QRegExp> >(type_id, keywords));
        }
